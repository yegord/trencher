...
void reduce(const Program &program, Program &resultProgram, Thread *attacker, Transition *attackWrite, Transition *attackRead, 
	...
	const std::shared_ptr<Constant>  attackAddr = resultProgram.makeConstant(0);
	const std::shared_ptr<Constant>  attackingAddr = resultProgram.makeConstant(1);
	const std::shared_ptr<Constant>  successAddr    = resultProgram.makeConstant(2);
	resultProgram.setInteresting(successAddr->value(), SERVICE_SPACE);
  ...
  const std::shared_ptr<Condition> check_not_blocked(new Condition(std::make_shared<NotBlocked>()));
	const std::shared_ptr<Condition> check_is_buffered(new Condition(std::make_shared<BinaryOperator>(BinaryOperator::EQ, tmp, one)));
	const std::shared_ptr<Condition> check_is_not_buffered(new Condition(std::make_shared<BinaryOperator>(BinaryOperator::EQ, tmp, zero)));
	const std::shared_ptr<Condition> check_access_is_write(new Condition(std::make_shared<BinaryOperator>(BinaryOperator::EQ, tmp, hb_write)));
	const std::shared_ptr<Condition> check_access_is_not_write(new Condition(std::make_shared<BinaryOperator>(BinaryOperator::NEQ, tmp, hb_write)));
	const std::shared_ptr<Condition> check_access_is_read_or_write(new Condition(std::make_shared<BinaryOperator>(BinaryOperator::NEQ, tmp, zero)));
			...
			if (thread == attacker || attacker == NULL) {
				...
				if (Write *write = transition->instruction()->as<Write>()) {
					if (transition == attackWrite || attackWrite == NULL) {
						...
						resultThread->makeTransition(
							...
							attackerTo,
							std::make_shared<Atomic>(
								std::make_shared<Read> (tmp,              attackingAddr,    SERVICE_SPACE),
								std::make_shared<Condition>(std::make_shared<BinaryOperator>(BinaryOperator::EQ, tmp, zero)),
								...
								std::make_shared<Write>(one,              attackingAddr,    SERVICE_SPACE)
							)
						);
					}
				}
				...
				if (fenced.find(transition->from()) != fenced.end()) {
					...
					resultThread->makeTransition(
						...
						std::make_shared<Atomic>(
					    check_not_blocked,
          		std::make_shared<Read>(tmp, read->address(), IS_BUFFERED_SPACE),
							check_is_buffered,
        ...
        /* One can become a helper only on a read or a write. */
				if (Read *read = transition->instruction()->as<Read>()) {
					resultThread->makeTransition(
						...
						std::make_shared<Atomic>(
					    check_not_blocked,
							std::make_shared<Read>(tmp, read->address(), HB_SPACE),
							check_access_is_write,
							transition->instruction()
						)
					);
				} else if (Write *write = transition->instruction()->as<Write>()) {
					resultThread->makeTransition(
						...
						std::make_shared<Atomic>(
					    check_not_blocked,
							std::make_shared<Read>(tmp, write->address(), HB_SPACE),
							check_access_is_read_or_write,
							transition->instruction(),
				...
				if (Read *read = transition->instruction()->as<Read>()) {
					...
					resultThread->makeTransition(
						...
						std::make_shared<Atomic>(
					    check_not_blocked,
							std::make_shared<Read>(tmp, attackAddr, SERVICE_SPACE),
							std::make_shared<Condition>(std::make_shared<BinaryOperator>(BinaryOperator::EQ, tmp, read->address())),
					...
					resultThread->makeTransition(
					    ...
					    check_not_blocked,
							std::make_shared<Read>(tmp, attackAddr, SERVICE_SPACE),
							std::make_shared<Condition>(std::make_shared<BinaryOperator>(BinaryOperator::EQ, tmp, write->address())),
							std::make_shared<Write>(one, successAddr, SERVICE_SPACE)
						)
					);
				} else if (transition->instruction()->as<Atomic>()) {
